let lastCarIndex = -1; // Pichhli tasveer ka index yaad rakhne ke liye
...
function onSwapPic() {
    ...
    let newIndex;
    do {
        newIndex = Math.floor(Math.random() * randomCars.length);
    } while (newIndex === lastCarIndex); // Jab tak naye index alag na ho, tab tak doosra index nikalo

    lastCarIndex = newIndex; // Naya index save kar lo
    image.src = randomCars[newIndex]; // Tasveer update karo
    image.className += " big";
}